...
function getEnvDev() {
    ...
    const nameExposes = allDotEnv.filter((val) => val?.includes("NAME_COMPONENT_EXPOSE"));
    const fileExposes = allDotEnv.filter((val) => val?.includes("SRC_COMPONENT_EXPOSE"));
    let exposes = {};
    ...
    const portRemotes = allDotEnv.filter((val) => val?.includes("REMOTE_PORT_APP_PROD"));
    const endpointRemotes = allDotEnv.filter((val) => val?.includes("REMOTE_ENDPOINT_PROD"));
